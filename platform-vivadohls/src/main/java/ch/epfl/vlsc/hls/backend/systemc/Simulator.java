...
public interface Simulator {
    ...
    default void genrateSimulator(Network network) {
        ...
        emitter().emit("int sc_main(int argc, char *argv[]) {");
        {
            ...
            emitter().emit("std::unique_ptr<Network> mut = std::make_unique<Network>(\"network\", period, opts.queue_capacity, opts.trace_level, !opts.profile_dump.empty());");
    ...
    default LogicVector getSCPortType(PortDecl port) {
        ...
        return ObjectFactory.getNewLogicVector(bitWidth);
    }
